export const airportOptions = [
  "Abha International Airport (AHB) - Abha",
  "Abu Dhabi International Airport (AUH)",
  "Al Ahsa International Airport (HOF) - Al Ahsa",
  "Al Ain International Airport (AAN)",
  "Al Maktoum International Airport (DWC)",
  "Amsterdam Airport Schiphol - AMS",
  "Amsterdam Airport Schiphol (AMS)",
  "Amsterdam Airport Schiphol (AMS) - Netherlands",
  "Athens International Airport Eleftherios Venizelos - ATH",
  "Athens International Airport Eleftherios Venizelos (ATH)",
  "Athens International Airport Eleftherios Venizelos (ATH) - Greece",
  "Auckland Airport (AKL) - New Zealand",
  "Barcelona-El Prat Airport - BCN",
  "Barcelona-El Prat Airport (BCN)",
  "Barcelona-El Prat Airport (BCN) - Spain",
  "Beijing Capital International Airport (PEK) - China",
  "Ben Gurion Airport (TLV) - Israel",
  "Berlin Brandenburg Airport (BER) - Germany",
  "Boston Logan International Airport (BOS) - United States",
  "Brussels Airport - BRU",
  "Brussels Airport (BRU)",
  "Brussels Airport (BRU) - Belgium",
  "Budapest Ferenc Liszt International Airport - BUD",
  "Budapest Ferenc Liszt International Airport (BUD) - Hungary",
  "Charles de Gaulle Airport (CDG) - France",
  "Chhatrapati Shivaji Maharaj International Airport (BOM) - India",
  "Cologne Bonn Airport (CGN) - Germany",
  "Copenhagen Airport - CPH",
  "Copenhagen Airport (CPH)",
  "Copenhagen Airport (CPH) - Denmark",
  "Dallas/Fort Worth International Airport (DFW) - United States",
  "Denver International Airport (DEN) - United States",
  "Detroit Metropolitan Wayne County Airport (DTW) - United States",
  "Doha Hamad International Airport (DOH) - Qatar",
  "Dubai International Airport (DXB)",
  "Dubai International Airport (DXB) - United Arab Emirates",
  "Dubai World Central - Al Maktoum International Airport (DWC) - UAE",
  "Dublin Airport - DUB",
  "Dublin Airport (DUB)",
  "Dublin Airport (DUB) - Ireland",
  "Düsseldorf Airport (DUS)",
  "Düsseldorf Airport (DUS) - Germany",
  "Frankfurt Airport - FRA",
  "Frankfurt Airport (FRA)",
  "Frankfurt Airport (FRA) - Germany",
  "Fujairah International Airport (FJR)",
  "Geneva Airport (GVA) - Switzerland",
  "Guangzhou Baiyun International Airport (CAN) - China",
  "Hamad International Airport",
  "Hamburg Airport (HAM) - Germany",
  "Hartsfield-Jackson Atlanta International Airport (ATL) - United States",
  "Heathrow Airport (LHR) - United Kingdom",
  "Helsinki-Vantaa Airport - HEL",
  "Helsinki-Vantaa Airport (HEL)",
  "Helsinki-Vantaa Airport (HEL) - Finland",
  "Hong Kong International Airport (HKG) - Hong Kong",
  "Incheon International Airport (ICN) - South Korea",
  "Indira Gandhi International Airport (DEL) - India",
  "Istanbul Airport (IST) - Turkey",
  "Istanbul Sabiha Gökçen International Airport (SAW) - Turkey",
  "John F. Kennedy International Airport (JFK) - United States",
  "King Abdulaziz International Airport (JED) - Jeddah",
  "King Abdulaziz International Airport (JED) - Saudi Arabia",
  "King Fahd Air Base (OERK) - Dammam",
  "King Fahd International Airport (DMM) - Dammam",
  "King Khalid International Airport (RUH) - Riyadh",
  "Kuala Lumpur International Airport (KUL) - Malaysia",
  "Kuwait International Airport (KWI)",
  "Leonardo da Vinci–Fiumicino Airport (FCO) - Italy",
  "Lisbon Airport (LIS) - Portugal",
  "Lisbon Humberto Delgado Airport - LIS",
  "Lisbon Humberto Delgado Airport (LIS)",
  "London Gatwick Airport (LGW) - United Kingdom",
  "London Heathrow Airport - LHR",
  "London Heathrow Airport (LHR)",
  "London Heathrow Airport (LHR) - United Kingdom",
  "London Luton Airport (LTN) - United Kingdom",
  "Los Angeles International Airport (LAX) - United States",
  "Luxembourg Airport (LUX) - Luxembourg",
  "Lyon-Saint Exupéry Airport (LYS) - France",
  "Madrid-Barajas Adolfo Suárez Airport - MAD",
  "Madrid-Barajas Adolfo Suárez Airport (MAD)",
  "Madrid-Barajas Adolfo Suárez Airport (MAD) - Spain",
  "Málaga Costa del Sol Airport (AGP) - Spain",
  "Malta International Airport (MLA)",
  "Manchester Airport (MAN) - United Kingdom",
  "McCarran International Airport (LAS) - United States",
  "Miami International Airport (MIA) - United States",
  "Milan Malpensa Airport (MXP) - Italy",
  "Minneapolis-Saint Paul International Airport (MSP) - United States",
  "Munich Airport - MUC",
  "Munich Airport (MUC)",
  "Munich Airport (MUC) - Germany",
  "Nice Côte d'Azur Airport (NCE) - France",
  "Ninoy Aquino International Airport (MNL) - Philippines",
  "O'Hare International Airport (ORD) - United States",
  "Oslo Airport Gardermoen (OSL)",
  "Oslo Airport, Gardermoen (OSL) - Norway",
  "Paris Charles de Gaulle Airport - CDG",
  "Paris Charles de Gaulle Airport (CDG)",
  "Paris Charles de Gaulle Airport (CDG) - France",
  "Paris Orly Airport (ORY) - France",
  "Philadelphia International Airport (PHL) - United States",
  "Porto Airport (OPO) - Portugal",
  "Prague Václav Havel Airport - PRG",
  "Prague Václav Havel Airport (PRG) - Czech Republic",
  "Prince Mohammad bin Abdulaziz International Airport (MED) - Medina",
  "Prince Nayef bin Abdulaziz International Airport (ELQ) - Qassim",
  "Prince Sultan Air Base (OEKQ) - Kharj",
  "Ras Al Khaimah International Airport (RKT)",
  "Rome Fiumicino Airport - FCO",
  "Rome Leonardo da Vinci-Fiumicino Airport (FCO)",
  "Rome Leonardo da Vinci-Fiumicino Airport (FCO) - Italy",
  "San Diego International Airport (SAN) - United States",
  "San Francisco International Airport (SFO) - United States",
  "Seattle-Tacoma International Airport (SEA) - United States",
  "Seoul Incheon International Airport (ICN) - South Korea",
  "Shanghai Hongqiao International Airport (SHA) - China",
  "Shanghai Pudong International Airport (PVG) - China",
  "Sharjah International Airport (SHJ)",
  "Sheremetyevo International Airport (SVO) - Russia",
  "Singapore Changi Airport (SIN) - Singapore",
  "Stockholm Arlanda Airport - (ARN)",
  "Stockholm Arlanda Airport (ARN)",
  "Stockholm Arlanda Airport (ARN) - Sweden",
  "Stuttgart Airport (STR) - Germany",
  "Suvarnabhumi Airport (BKK) - Thailand",
  "Sydney Airport (SYD) - Australia",
  "Ta'if Regional Airport (TIF) - Ta'if",
  "Tokyo Haneda Airport (HND) - Japan",
  "Toronto Pearson International Airport (YYZ) - Canada",
  "Vancouver International Airport (YVR) - Canada",
  "Vienna International Airport - VIE",
  "Vienna International Airport (VIE)",
  "Vienna International Airport (VIE) - Austria",
  "Warsaw Chopin Airport - WAW",
  "Warsaw Chopin Airport (WAW)",
  "Warsaw Chopin Airport (WAW) - Poland",
  "Zurich Airport - ZRH",
  "Zurich Airport (ZRH)",
  "Zurich Airport (ZRH) - Switzerland"
];
